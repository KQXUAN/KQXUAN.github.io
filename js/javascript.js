...
function FightAgainstLandlords(player_obj,difen,Ama,poker,another,special,cardSE,Music,Rob){
	...
	this.Animate = new Animation(Ama,special,poker,Music,Rob,cardSE);//动画函数
	console.log(this.Animate);
	this.SetInfo(player_obj);
	...
	this.InitEvents(poker,another,special,Music,cardSE,Ama,Rob);		//绑定事件
	this.DefineVariable();
	// this.Game();		//游戏主体
	// this.specialEffects.scoreboard(special);

	// this.Animation.spotMove(Ama);
	// this.Animation.stripMove(Ama,special);
...
//初始化牌堆的方法
	...
	//绑定事件的方法
	InitEvents:function(poker,another,special,Music,cardSE,Ama,Rob){
	 	var that = this;
		...
		poker.$allPoker.on('click',poker.$allPoker,function(){
			...
			that.Animate.shuffleCard(poker,Music,special,Rob,cardSE,that);
			// that.shuffleCard(poker,Music,special,Rob,cardSE);
		...
		Rob.$rob_1_1.on('click', function(){
			...
			if(that.rob_arr.indexOf(2) == -1){	//没出现过2说明没人叫过地主 
				...
				that.addPoke(poker,special,Music,cardSE);
			...
			if(that.rob_arr[1] != 0){		//下一个玩家没放弃过抢地主
				...
				that.robTime(2,Music,Rob,poker,special,cardSE);
				...
				that.robTime(3,Music,Rob,poker,special,cardSE);
		...
		Rob.$rob_1_2.on('click', function(){
			that.noRob(1,Rob,Music,poker,special,cardSE);
		});
		...
		Rob.$rob_2_1.on('click', function(){
			...
			if(that.rob_arr[2] != 0){		//下一个玩家没放弃过抢地主
				...
				that.robTime(1,Music,Rob,poker,special,cardSE);
		...
		Rob.$rob_2_2.on('click', function(){
			that.noRob(2,Rob,Music,poker,special,cardSE);
		});
		...
		Rob.$rob_3_2.on('click', function(){
			that.noRob(3,Rob,Music,poker,special,cardSE);
		});
		...
		poker.$play_2_bt.click(function(){
			...
			that.playSend(poker,special,Music,cardSE);
		...
		poker.$play_2_bt2.click(function(){
			...
			that.playPass(poker,special,Music);
		...
		another.$left_people.click(function(){
	 	...
	 	another.$left_set1_span.on('click',function(){   //连对
	 		console.log(cardSE);
	 	...
	 	another.$left_set2_span.on('click',function(){   //炸弹
	 	...
	 	var lu = 0;
	 	another.$right_set1_span.on('click',function(){ //顺子
			...
			Music.sound_01.play();
	 		...
	 		that.Animate.cardSpecialEffect.straight(cardSE);
	 	})
	 	...
	 	another.$right_set2_span.on('click',function(){  //飞机
			...
			Music.sound_02.play();
	 		...
	 		that.Animate.cardSpecialEffect.planet(cardSE,Ama);
	 	})
	...
	//定义游戏音效的方法
	Sound:function(special,Music,cardSE){
		...
		switch(type){
			...
			case 100:
				...
				this.Animate.cardSpecialEffect.bomb(cardSE);
			...
			case 6:
				...
				this.Animate.cardSpecialEffect.straight(cardSE);
			...
			case 66:
				...
				this.Animate.cardSpecialEffect.lian(cardSE);
			...
			case 110:
				...
				this.Animate.cardSpecialEffect.wz(special,cardSE);
		...
		if(type == 1122 || type == 1123){
			...
			this.Animate.cardSpecialEffect.planet(cardSE);
		...
		setTimeout(function(poker){	
			...
			setTimeout(function(){
				...
				if(number<17){
					...
					setTimeout(function(){
						that.straightenPoker(poker,Music,special,Rob,cardSE);  //等待发完牌后调用将牌归正的函数方法
					},300)
	...
	/*straightenPoker:function (poker,Music,special,Rob,cardSE){
		var that = this;
	...
	//---------对3副牌已经排好序的手牌展开----------------
		...
		setTimeout(function(poker,Music,special,Rob,cardSE){
			...
			//开始抢地主
			that.robSui(Music,special,Rob,cardSE);

		},1250,poker,Music,special,Rob);
	...
	robSui:function(Music,special,Rob,cardSE){
		for(var i=1; i<4; i++){
		...
		this.robTime(math,Music,Rob,special,cardSE);
	},

	//添加地主牌到玩家中并在页面上显示出来
	addPoke:function(poker,special,Music,cardSE){
		...
		if(m == 1){
			...
			this.sendTime(poker,special,Music,cardSE);
		...
		//如果不是玩家2抢到地主的话自动打牌
		setTimeout(function(special,Music,cardSE){

			if(m == 0){

				that.PromptPoke(that.all_player[0].poker,0);

				that.npcSend(0,poker,special,Music,cardSE);
				...
				that.npcSend(2,poker,special,Music,cardSE);
				...
				setTimeout(function(){
					...
					if(that.ready_obj.poker.length != 0){
						...
						that.npcPass(0,poker,special,Music,cardSE);
	...
	//定义一个用来抢地主计时的方法 
	robTime:function(t,Music,Rob,poker,special,cardSE){
		...
		this.int = setInterval(function(){
			...
			if(num < 0 && that.rob_arr.indexOf(true) == -1){//放弃
				...
				that.noRob(t,Rob,Music,poker,special,cardSE);		//时间到就不抢地主
	...
	//点击不抢地主的方法
	noRob:function(num,Rob,Music,poker,special,cardSE){
		...
		if(num == 1){
			...
			if(this.rob_arr[0]==0 && this.rob_arr[1]==0 && this.rob_arr[2]==0){	//说明所有人都不抢
				...
				this.addPoke(poker,special,Music,cardSE)
				...
				Rob.$rob_2_1.attr('value','不叫');

				Rob.$rob2.css('display','block');

				clearInterval(this.int);

				this.robTime(2,Music,Rob,poker,special,cardSE);
			...
			if(this.rob_arr[0]==0 && this.rob_arr[1]==0 && this.rob_arr[2]==0){	//说明所有人都不抢
				...
				this.robTime(3,Music,Rob,poker,special,cardSE);
			...
			if(this.rob_arr[0]==0 && this.rob_arr[1]==0 && this.rob_arr[2]==0){	//说明所有人都不抢
				...
				this.robTime(1,Music,Rob,poker,special,cardSE);
	...
	//定义玩家出牌的方法
	playSend:function(poker,special,Music,cardSE){
		...
		this.Sound(special,Music,cardSE);
		...
		setTimeout(function(){
			...
			this.ready_obj = {poker:[],max:0,type:0};   //出完牌后将出牌数组清空
		...
		if(this.all_player[1].poker.length == 0){
			...
			if(this.all_player[1].role == 1){
				...
				this.all_player[0].integral = Number(this.all_player[0].integral) - Number(this.bei*this.difen);

				this.all_player[1].integral = Number(this.all_player[1].integral) + Number(this.bei*this.difen);

				this.all_player[2].integral = Number(this.all_player[2].integral) - Number(this.bei*this.difen); 
		...
		//延时电脑提示出牌
		setTimeout(function(special,Music){

			that.PromptPoke(that.all_player[2].poker,2);			//调用提示方法

			if(that.ready_obj.poker.length == 0){			//提示出来是有牌可以出的

				that.npcPass(2,poker,special,Music,cardSE);								//那就不出咯

			}else {

				that.npcSend(2,poker,special,Music,cardSE);								//右边电脑出牌
			...
			setTimeout(function(){
				...
				if(that.ready_obj.poker.length == 0){			//提示出来是没有牌可以出的
					...
					that.npcPass(0,poker,special,Music,cardSE);								//那就不出咯

				}else {

					that.npcSend(0,poker,special,Music,cardSE);								//左边电脑出牌
	...
	//定义玩家不出牌的方法
	playPass:function(poker,special,Music){
	...
	//定义一个用来出牌计时的方法
	sendTime:function(poker,special,Music,cardSE){
		...
		this.set = setInterval(function(){
			...
			if(number < 0 && this.cont >= 2){//放弃
				...
				that.playSend(poker,special,Music,cardSE);									//调用玩家出牌方法;		
				...
				that.playPass(poker,special,Music);		//调用玩家不出牌的方法
	...
	//定义电脑出牌的方法
	npcSend:function(num,poker,special,Music,cardSE){
		...
		if(this.all_player[num].poker.length == 0){	//在此处添加结束动画，显示积分榜
			...
			if(this.all_player[num].role == this.all_player[1].role){
				...
				// console.log(this.rob_arr);
				this.specialEffects.lose(special,Music,this.rob_arr);
	...
	//定义电脑的不出牌方法
	npcPass:function(num,poker,special,Music,cardSE){	
	...
	specialEffects:{
		//-------------------积分榜动画--------------	
		scoreboard:function(special) {
			// console.log(this.all_player);
			special.$num1td.eq(1).html(this.all_player[0].integral);
			special.$num1td.eq(2).html(victery_01);

			special.$num2td.eq(1).html(this.all_player[1].integral);
			special.$num2td.eq(2).html(victery_02);

			special.$num3td.eq(1).html(this.all_player[2].integral);
			special.$num3td.eq(2).html(victery_03);
		...
		win:function (special,Music){
			...
			special.$win.click(function(){
				...
				that.scoreboard(special);		//在此处添加结束动画，显示积分榜

			})
		},
		//------------------------玩家输的时候效果------------------
		
		lose:function (special,Music,rob_arr){
			var that = this;
			...
			setTimeout(function(special){
				...
				special.$lose.click(function(special){
					...
					that.specialEffects.scoreboard();		//在此处添加结束动画，显示积分榜
	...
	ToDown:function(obj){
		...
		console.log(this.ready_obj.poker);
	...
	Replay:function(){
		...
		// game();
		// new FightAgainstLandlords(player_obj,difen,Ama,poker,another,special,cardSE,Music,Rob);
	}	
}
