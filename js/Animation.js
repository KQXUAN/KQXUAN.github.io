function Animation(Ama,special,poker,Music,Rob,cardSE){
...
Animation.prototype = {
		...
		stripMove:function (Ama,special,Music){
			...
			Ama.$strip.click(function(){   
				if(Ama.$write.html() == 'START'){   //要判断是否已经加载完才能进行点击
					...
					// Ama.$music.attr({src:'./music/back/back_01.mp3',loop:false});
	...
	shuffleCard:function(poker,Music,special,Rob,cardSE,That){
			
		
		var that = this;          
			// console.log(poker.$allPokerLi)
		for(var i=53;i>=0;i--){
			(function(j){
				setTimeout(function(){
						// console.log(j);
					...
					setTimeout(function(){
						...
						if(j == 0){
							setTimeout(function(){
								...
								that.sendPoker(0,poker,Music,special,Rob,That);
 	...
 	sendPoker:function (number,poker,Music,special,Rob,That){
 		var that = this;
		...
		setTimeout(function(poker){
			// console.log(poker.$allPokerLi.eq(poker.$allPokerLi.length-1))
		...
		setTimeout(function(poker,That){
			...
			// console.log(that.all_poker_data.pop())
		...
		setTimeout(function(poker,That){	
			...
			setTimeout(function(){
				...
				if(number<17){
					that.sendPoker(number,poker,Music,special,Rob,That);  //自调用发牌方法
				
				}else{
					
					setTimeout(function(){
						that.straightenPoker(poker,Music,special,Rob,That);  //等待发完牌后调用将牌归正的函数方法
					},300)
	...
	straightenPoker:function (poker,Music,special,Rob,That){
		var that = this;
	...
	//---------对3副牌已经排好序的手牌展开----------------
		...
		setTimeout(function(poker,Music,special,Rob,That){
			...
			//开始抢地主
			That.robSui(Music,special,Rob);

		},1250,poker,Music,special,Rob,That);
	...
	mid:function(poker,That){
		...
		if(poker.$cardMid.remove()){  //将中间玩家的手牌删掉
			...
			for(var j=0;j<length;j++){	   //循环生成17张牌的牌面
				// console.log('this.all_player['+j+'].poker')
	...
	//定义移动牌的函数方法
	// console.log(poker.$cardMid.length)
